...
def login(
    ...
    user_db = crud.get_user_by_email(db, login_data.email)
    if not user_db or not user_db.verify_password(login_data.password):
        raise HTTPException(
...
def create_superuser_endpoint(
    user: UserCreate,
    db: Session = Depends(get_db)
):
    """Create a superuser account."""
    return crud.create_superuser(
        db=db,
        email=user.email,
