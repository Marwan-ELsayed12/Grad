...
@router.get("/borrowed", response_model=List[Order])
def read_borrowed_orders(
    db: Session = Depends(deps.get_db),
    skip: int = 0,
    limit: int = 100,
    current_user = Depends(deps.get_current_user)
...
@router.get("/purchased", response_model=List[Order])
def read_purchased_orders(
    db: Session = Depends(deps.get_db),
    skip: int = 0,
    limit: int = 100,
    current_user = Depends(deps.get_current_user)
